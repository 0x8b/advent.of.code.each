...
def get_all_trails(topo, trailhead_row, trailhead_col):
    rows = len(topo)
    cols = len(topo[0])
    ...
    def hike(row, col, height, trail):
        if not (0 <= row < rows and 0 <= col < cols) or topo[row][col] != height:
            return
...
def count_trails(topo, distinct=False):
    ...
    for trailhead_row, trailhead_col in find_trailheads(topo):
        trails = get_all_trails(topo, trailhead_row, trailhead_col)
